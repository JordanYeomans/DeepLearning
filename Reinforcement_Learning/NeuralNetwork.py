...
class NeuralNetwork:
    ...
    def __init__(self, num_actions, state_shape, checkpoint_dir, worker=False):
        ...
        self.update_next_version_checkpoint_dir(checkpoint_dir)
    ...
    def load_checkpoint(self, reset_if_error=True):
        """
        ...
        try:
            ...
            if reset_if_error:
                ...
                print("Failed to restore checkpoint from:", self.checkpoint_dir)
                print("Initializing variables instead.")
                self.session.run(tf.global_variables_initializer())
            else:
                print('Error Restoring Checkpoint')
    ...
    def save_checkpoint(self):
        ...
        self.saver.save(self.session,
                        save_path=self.next_checkpoint_path,
                        global_step=1)
    ...
    def get_q_values(self, states):
        ...
        Old: # A single state contains two images (or channels): The most recent
        Old: # image-frame from the game-environment, and a motion-tracing image.
        Old: # See the MotionTracer-class for details.
    ...
    def update_checkpoint_dir(self, checkpoint_dir):
        ...
        # Path for restoring checkpoints.
        self.checkpoint_path = os.path.join(checkpoint_dir, "checkpoint")

    def update_next_version_checkpoint_dir(self, next_checkpoint_dir):
        self.next_checkpoint_dir = next_checkpoint_dir

        # Path for saving checkpoints.
        self.next_checkpoint_path = os.path.join(next_checkpoint_dir, "checkpoint")